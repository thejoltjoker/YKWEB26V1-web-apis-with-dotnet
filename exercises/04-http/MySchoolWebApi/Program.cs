...
{
    if(request.StartDate == default || request.EndDate == default) return Results.BadRequest("Date fields are required");
...
app.MapPut(("/courseinstances/{id}"), (string id, UpdateCourseInstanceRequest request) =>
{
    try
    {
        ...
        // Update data
        if (request.StartDate != null) result.StartDate = request.StartDate;
        if (request.EndDate != null) result.EndDate = request.EndDate;
